function [phi_demo, phi_k] = ioc_load_instance_remove_demo( nb_samples, nb_features, data_folder, demo_id, nb_demos )

formatSpec = '%03d';

filename_data = [data_folder 'spheres_features_', num2str(nb_samples,formatSpec), '.txt'];
filename_ids = [data_folder 'spheres_features_demo_ids_', num2str(nb_samples,formatSpec), '.txt'];
...
index = 1;
for d=1:size(ids),
    % only adds the rows that do not belong
    % to that demo id
    if ( ids(d) ~= demo_id ) && ids(d) >= 0, 
        phi_k(:,:,index) = m(((d-1)*nb_samples+(nb_demo+1)):(d*nb_samples+nb_demo),:);
        index = index + 1;
    end
end